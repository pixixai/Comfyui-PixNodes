...
| 索引 | 输出名 | 类型 | 描述 |
|---|---|---|---|
| **0** | **video_batch** | `VIDEO` (List) | **视频批次（自动循环）**。<br>输出一个包含多个 `VIDEO` 对象的列表。当连接到下游节点（如 `Get Video Components` 或 `Save Video`）时，ComfyUI 会自动触发**批处理循环**，依次处理列表中的每一个视频。 |
| **1** | **video_paths_json** | `JSON` | **视频路径数据 (JSON)**。<br>输出一个包含所有视频绝对路径的 JSON 数组（例如 `["E:/a.mp4", "E:/b.mp4"]`）。<br>与 `video_batch` 不同，**此输出不会被拆分**。无论批处理循环运行多少次，这个端口每次都会输出完整的列表数据，适合需要一次性获取所有文件信息的自定义节点使用。 |
