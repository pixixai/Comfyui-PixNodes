...
- **跳过未连接 (skip_unconnected)**:
    - 类型：`BOOLEAN`
    - 说明：控制当某个 Key 对应的端口没有连接时的行为。
        - **开启 (True)**: 忽略该 Key，不写入结果（默认）。
        - **关闭 (False)**: 保留该 Key，并将其值设为空字符串 `""`。
...
### 场景 2：跳过未连接 (Skip Unconnected)

假设 `custom_value` 未连接。

- **当 `skip_unconnected = True` (开启)**:
