...
class QwenApiNode(QwenLLMBase):
    """
    ComfyUI Qwen API Node - PixNodes Series
    Saves API keys to ComfyUI/user/PixNodes/ for persistence.
    Supports Qwen-VL Vision models.
    """
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "model": (["qwen-max", "qwen-plus", "qwen-flash", "qwen-vl-max", "qwen-vl-plus", "qwen-vl-ocr"], {"default": "qwen-plus"}),
    ...
    def call_qwen(self, **kwargs):
        ...
        final_key = self._get_or_save_key("qwen", api_key)
        if not final_key:
            return ("Error: Qwen API Key not found.", "")
        ...
        try:
            ...
            PromptServer.instance.send_sync("qwen_status", {"status": "success", "node": "Pix_QwenApiNode"})
            return (result, new_history)

        except AuthenticationError:
            return ("Error: Qwen API Key invalid.", "")
...
# 注册映射：统一使用 Pix_ 前缀
NODE_CLASS_MAPPINGS = {
    "Pix_QwenApiNode": QwenApiNode
}

# 显示名称映射：使用英文标识，由 JSON 负责汉化
NODE_DISPLAY_NAME_MAPPINGS = {
    "Pix_QwenApiNode": "Qwen API Assistant (PixNodes)"
}